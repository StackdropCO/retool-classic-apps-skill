...
def validate_app(app_dir):
    ...
    # -----------------------------------------------------------------------
    # 3b. No raw double quotes inside ="{{ ... }}" attribute expressions.
    # A `"` inside a double-quoted attribute terminates the attribute early and
    # Retool's importer then fails the WHOLE import silently — no error, no new
    # app. (Found the hard way 2026-08-18: + "'s budget" inside a label.)
    # Use single-quoted JS strings inside expressions; for an apostrophe use a
    # typographic one ('\u2019s budget') rather than escaping.
    # -----------------------------------------------------------------------
    quote_errors = []
    for rsx_file in rsx_files:
        with open(rsx_file, encoding='utf-8') as f:
            content = f.read()
        for m in re.finditer(r'=\s*"\{\{((?:(?!\}\}).)*)\}\}"', content, re.S):
            if '"' in m.group(1):
                line = content[:m.start()].count('\n') + 1
                quote_errors.append(f'{os.path.basename(rsx_file)}:{line}')

    if quote_errors:
        vr.add('FAIL', f'Raw double quote inside attribute expression (silent import killer): {", ".join(quote_errors)}')
    else:
        vr.add('PASS', 'No double quotes inside attribute expressions')

    # -----------------------------------------------------------------------
    # 3c. No _comment on a <View>.
    # A View is a transparent grouping element with no component of its own. A
    # _comment on one SURVIVES an export but makes the importer fail the WHOLE
    # import silently. (Found by 2x2 bisection 2026-08-27 after nine days of
    # silent failures; _comment is fine on every other element.)
    # Explain a View in an ADR instead.
    # -----------------------------------------------------------------------
    view_comment_errors = []
    for rsx_file in rsx_files:
        with open(rsx_file, encoding='utf-8') as f:
            content = f.read()
        for m in re.finditer(r'<View\b((?:[^<>]|\{[^{}]*\})*?)/?>', content, re.S):
            if '_comment' in m.group(1):
                line = content[:m.start()].count('\n') + 1
                view_comment_errors.append(f'{os.path.basename(rsx_file)}:{line}')

    if view_comment_errors:
        vr.add('FAIL', f'_comment on a <View> (silent import killer): {", ".join(view_comment_errors)}')
    else:
        vr.add('PASS', 'No _comment on View elements')

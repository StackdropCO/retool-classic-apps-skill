...
## 6. Critical Import Failures (visible errors)
...
## 6a. Silent Import Killers

A different failure class from §6: the importer produces **nothing** — no
error, no toast, no change. And since importing into an app **replaces it in
place**, "silently failed" and "succeeded with no visible change" look
identical. Two constructs are proven (by bisection against real imports) to
cause this:

1. **A raw `"` inside a `="{{ … }}"` attribute expression.** The quote
   terminates the attribute early. Use single-quoted JS strings inside
   expressions; for an apostrophe use a typographic `’` (`'…’s budget'`),
   never an escaped `"`.
2. **`_comment` on a `<View>`.** Fine on every other element. It even
   *survives an export* — surviving export ≠ accepted on import — so Retool
   can hand you a zip that will not re-import.

`validate_app.py` fails the build on both. Also: zips must be **flat**
(main.rsx at the archive root — a wrapper folder can no-op the import), and
an import you must be certain about deserves a **probe marker** (a loud
title/background change) so success is observable. Full model, probe method
and the bisection procedure for a new unknown killer:
...
## 6d. Production Query & State Pitfalls

Learned from production incidents; each of these fails quietly or corrupts data.

- **Sticky-row State.** A pane or single-row mutation acting on a row from a
  *filtered* table must read a State snapshot written on row-select — never
  `table.selectedRow`. The moment the row stops matching the filter (e.g. its
  status changes), the selection empties mid-gesture: the pane blanks and the
  mutation targets nothing or the wrong row. Bulk actions may read the live
  selection; single-row flows snapshot.
- **No explicit triggers on filter controls.** If the query's SQL references a
  filter widget's value, Retool already re-runs it reactively. Adding a change
  event on top double-fires it and Retool cancels one run as
  "duplicate query run".
- **A manual query that reads AND writes the same State** must have
  `runWhenModelUpdates={false}`, or writing the State re-triggers the query
  forever.
- **Never pass an array into a SQL parameter** (`= ANY({{ arr }})`,
  `IN ({{ arr }})`). Retool binds it as one value but emits one `$n` per
  element → "bind message supplies N parameters, but prepared statement
  requires M". Use
  `IN (SELECT jsonb_array_elements_text({{ JSON.stringify(arr) }}::jsonb))`.
- **Never put `{{ }}` inside a SQL comment.** Retool's parameter scanner is
  textual and binds it anyway — placeholder/value mismatch.
- **Never `encodeURIComponent` a value in a REST query string** that Retool
  interpolates (emails especially). Retool encodes it already; a second pass
  sends `%2540` for `@` and matches nothing.
- **Guards live in the SQL statement, not only in the button.** A disabled
  button reads a stale snapshot; the SQL's WHERE reads the truth at execution
  (`AND status = 'pending'` makes a double-click a 0-row no-op). Warn on
  0 rows in the success handler.
- **Idempotent writes send absolutes, not deltas.** A retried "add 50" adds
  100; a retried "set to 250" sets 250. Compute the final value once, store
  it, send it verbatim on every retry.
...
**Filtered tables:** `table.selectedRow` here is only safe when the table is
unfiltered. If a filter can drop the row mid-gesture (status change, search),
snapshot the row into a State on select and point `filterBy` at the State —
see §6d "Sticky-row State".

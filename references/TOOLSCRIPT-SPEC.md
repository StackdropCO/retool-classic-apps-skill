...
⚠️ **Never put `_comment` on a `<View>`.** It survives a Retool export — so an
export can hand you back a zip — but the importer then fails the whole import
**silently** (no error, no change). Proven by 2×2 bisection after nine days of
silent import failures; `_comment` round-trips fine on every other element.
`validate_app.py` fails the build on it. A View that needs explaining is
explaining a decision — put the why in the repo's docs instead.
...
-- Array membership — NEVER `= ANY({{ arr }})` or `IN ({{ arr }})`: Retool
-- binds the array as ONE value but emits one $n placeholder per element, so
-- 2+ items fail with "bind message supplies N parameters, but prepared
-- statement requires M". Pass it as JSON and expand server-side:
WHERE id IN (SELECT jsonb_array_elements_text({{ JSON.stringify(arrayVariable) }}::jsonb)::int)
...
1. Create the directory structure (see Section 2):
   ...
   └── lib/           (optional, for extracted code)
       ...
       └── script.js
   ```
2. Zip the directory **flat** — `main.rsx` and `metadata.json` at the archive
   ROOT, no wrapper folder. That is the shape Retool's own *Export to ZIP*
   produces; a wrapped zip can import as nothing at all, silently:
   ```bash
   (cd "My App" && zip -r "../My App.zip" . -x '*.DS_Store')
   ```
3. Import the zip in Retool. Two semantics to know:
   - **Importing into an existing app REPLACES it in place** (same identity,
     same Custom URL). **Create new → From file/ZIP** is the path that makes a
     *separate* app; its name comes from the import dialog, not the archive.
   - **A failed import is SILENT** — no toast, no error, nothing changes. Since
     "replaced with no visible change" and "failed to parse" look identical,
     verify with a probe (see `references/SILENT-FAILURES.md`).
...
<App>
  ...
  <Frame id="$main" type="main" padding="8px 12px" paddingType="normal" sticky={false}
    ...
    <Text id="placeholder" _comment="components here" value="…" />
  </Frame>
</App>
```
